...
export default function DashboardPage() {
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: C.cream }}>
      <p style={{ color: C.muted, fontSize: '14px', fontFamily: "'DM Sans', sans-serif", fontWeight: '300', letterSpacing: '0.5px' }}>
        Chargement…
      </p>
    </div>
  );
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.beige, fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {menuOpen && (
        <div style={{ background: C.cream, borderBottom: `1px solid ${C.light}`, padding: '16px 24px', display: 'flex', flexDirection: 'column', gap: '4px', position: 'sticky', top: '58px', zIndex: 99 }}>
          {[
            ...
            <a key={link.label} href={link.href} style={{ fontSize: '14px', color: C.dark, textDecoration: 'none', padding: '8px 12px', borderRadius: '6px', fontWeight: '500' }}>{link.label}</a>
          ))}
          <button onClick={handleLogout} style={{ marginTop: '8px', padding: '10px 12px', background: C.light, border: 'none', borderRadius: '6px', fontSize: '14px', color: C.dark, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", fontWeight: '600', textAlign: 'left' }}>
            Déconnexion
          </button>
        </div>
      )}
      ...
      {/* Contenu */}
      <div className="dash-content" style={{ maxWidth: '960px', margin: '0 auto', padding: '36px 24px' }}>
        ...
        <div style={{ ...fadeItem(0), marginBottom: '28px' }}>
          <h1 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: '30px', fontWeight: '700', color: C.dark, margin: '0 0 6px', letterSpacing: '-0.5px' }}>
            Bonjour, <em style={{ fontStyle: 'italic', color: C.caramel }}>{user?.shopName}</em>
          </h1>
          <p style={{ fontSize: '13px', color: C.muted, margin: 0, fontWeight: '300' }}>
            Votre lien :{' '}
            <a href={user?.shopUrl} rel="noreferrer" target="_blank" style={{ color: C.dark, fontWeight: '500', textDecoration: 'none', borderBottom: `1px solid ${C.dark}40`, paddingBottom: '1px' }}>
              {user?.shopUrl}
            </a>
          </p>
        </div>
        ...
        <div className="dash-stats-grid" style={{ marginBottom: '20px' }}>
          {[
            { label: 'Vues (30 j)',      value: stats?.last30Days?.pageViews    || 0, delay: 80  },
            { label: 'Clics WhatsApp',   value: stats?.last30Days?.whatsappClicks || 0, delay: 160 },
            { label: 'Produits',         value: stats?.productCount              || 0, delay: 240 },
          ].map(({ label, value, delay }) => (
            <div key={label} className="stat-card" style={{ ...cardStyle, ...fadeItem(delay) }}>
